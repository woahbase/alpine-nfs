...
client : # test mount in another container in local
	docker run --rm -it \
		...
		--net=host \
		-e NFSMODE=client \
		-v $(CURDIR)/root/defaults/fstab.nfs:/etc/fstab.nfs:ro \
		$(IMAGETAG) \
		$(CNTSHELL) \
		-ec \
...
$(shell docker run --rm --pull=never \
	--entrypoint rpc.mountd \
	$(IMAGETAG) \
	-v \
	2>&1 \
	| tail -n1 | sed -e 's_rpc.mountd version __')
endef
# -- }}}
